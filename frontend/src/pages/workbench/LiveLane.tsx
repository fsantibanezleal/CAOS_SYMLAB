  ...
  return (
    <div className="sym-live">
      <Callout variant="note" title={es ? 'La misma implementacion, presupuesto reducido' : 'The same implementation, reduced budget'}>
        <p>
          {es
            ? 'Esta busqueda ejecuta EL MISMO codigo que el horneado sin conexion, cargado en el navegador. No es una reimplementacion en JavaScript: mantener dos motores de acuerdo para siempre es exactamente el tipo de defecto que produce numeros publicados que nadie puede reproducir. Pero NO esperes el mismo resultado que la pestana Expresion, y la razon esta medida: Pyodide trae su propia compilacion de numpy (2.2.5 frente a 2.5.1 aqui), y con la misma semilla eso da un flujo aleatorio distinto, asi que esta corrida muestrea DATOS distintos. Mismo codigo, mismo presupuesto declarado, otras filas.'
            : 'This search runs THE SAME code as the offline bake, loaded into the browser. It is not a JavaScript reimplementation: keeping two engines in agreement forever is exactly the kind of defect that produces published numbers nobody can reproduce. But do NOT expect the same answer as the Expression tab, and the reason is measured rather than assumed: Pyodide ships its own numpy build (2.2.5 against 2.5.1 here), and at the same seed that yields a different random stream, so this run samples DIFFERENT rows. Same code, same declared budget, different data.'}
        </p>
        <p>
          {es
