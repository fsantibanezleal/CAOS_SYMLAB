  ...
  return (
    <div className="sym-live">
      <Callout variant="note" title={es ? 'La misma implementacion, presupuesto reducido' : 'The same implementation, reduced budget'}>
        <p>
          {es
            ? 'Esta busqueda ejecuta EL MISMO codigo que el horneado sin conexion, cargado en el navegador. No es una reimplementacion en JavaScript: mantener dos motores de acuerdo para siempre es exactamente el tipo de defecto que produce numeros publicados que nadie puede reproducir. El presupuesto aqui es menor, asi que espera una expresion mas simple que la de la pestana Expresion, no una ley distinta.'
            : 'This search runs THE SAME code as the offline bake, loaded into the browser. It is not a JavaScript reimplementation: keeping two engines in agreement forever is exactly the kind of defect that produces published numbers nobody can reproduce. The budget is smaller here, so expect a simpler expression than the Expression tab shows, not a different law.'}
        </p>
        <p>
          {es
