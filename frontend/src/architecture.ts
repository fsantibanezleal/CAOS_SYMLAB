...
The variant bar is the ablation. Each chip is a search configuration that differs from its neighbour by exactly ONE mechanism, so moving along the bar attributes a measured difference to a named change rather than to a bundle of changes. A case whose inputs carry no declared physical dimensions omits the unit-typed chip entirely, rather than showing a control that silently does nothing.

Four sub-tabs follow. Expression shows the discovered formula, its tree, and the validation views. Live runs the engine in your browser. Front and search shows the Pareto front, where clicking a point loads that expression into every other view, together with the convergence and diversity histories and the measured cost of the rung. Context is the deep write-up.
...
La barra de variantes es la ablacion. Cada chip es una configuracion de busqueda que difiere de su vecina en exactamente UN mecanismo, de modo que recorrer la barra atribuye una diferencia medida a un cambio con nombre y no a un paquete de cambios. Un caso cuyas entradas no declaran dimensiones fisicas omite por completo el chip de unidades, en lugar de mostrar un control que en silencio no hace nada.

Siguen cuatro sub-pestanas. Expresion muestra la formula descubierta, su arbol y las vistas de validacion. En vivo ejecuta el motor en tu navegador. Frente y busqueda muestra el frente de Pareto, donde hacer clic en un punto lo carga en todas las demas vistas, junto con las historias de convergencia y diversidad y el costo medido del escalon. Contexto es el desarrollo profundo.
