...
La lista de metodos de la barra lateral es la ablacion. Cada entrada es una configuracion de busqueda, y la mayoria anade exactamente UN mecanismo a la entrada anterior, de modo que recorrer la lista atribuye una diferencia medida a ese cambio con nombre. Tres no lo hacen, y decirlo es justamente el sentido de una ablacion: r2 activa a la vez el escalado lineal Y la guarda de intervalos, porque la contribucion de Keijzer son ambas; r6 activa la supervivencia edad-aptitud Y las islas, que son un mecanismo con dos interruptores; r7 activa los dos interruptores de deduplicacion. Ningun escalon APAGA nunca un mecanismo: antes r6 quitaba la supervivencia multiobjetivo y r7 la reponia, lo que dejaba sin atribucion la diferencia medida en r6, y ahora una prueba falla si eso vuelve. Lee esos tres como pasos compuestos. Un caso cuyas entradas no declaran dimensiones fisicas omite la entrada de unidades, en lugar de ofrecer una configuracion que en silencio no hace nada.
