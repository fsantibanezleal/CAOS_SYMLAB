...
`data-pipeline/symlab/io/contract.py`. Declares the required schema (columns, units, ranges) + an explicit
...
(run in CI) verifies index to manifests to artifacts exist, byte sizes match, and lane==gate. The web loads **only** these
